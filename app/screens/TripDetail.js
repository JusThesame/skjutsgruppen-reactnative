...
class TripDetail extends Component {
  ...
  componentWillReceiveProps({ trip, loading }) {
    ...
    if (!loading && trip.id) {
      if (trip.experienceStatus === EXPERIENCE_STATUS_PENDING) {
        ...
        const notificationMessage = trans(
          'detail.someone_has_already_created_an_experience',
        );
        this.setState({
          trip,
          loading,
  ...
  onSubmit = (comment, social) => {
    ...
    if (validation.pass()) {
      try {
        submit({ tripId: trip.id, text: comment, social })
          .then(() => {
  ...
  returnRideButton = () => {
    ...
    const icon = (
      <Image
        source={
          trip.type === FEED_TYPE_WANTED ? ReturnIconBlue : ReturnIconPink
        }
    ...
    if (trip.ReturnTrip.length === 1 && trip.Recurring.length < 1) {
      return (
        <TouchableOpacity
          ...
          onPress={() =>
            this.redirectToSelectedReturnTrip(trip.ReturnTrip[0].id)
          }
          activeOpacity={0.75}
        >
          {icon}
  ...
  recurringRidesModal = () => {
    ...
    return (
      <Modal
        ...
        transparent
        onRequestClose={() =>
          this.setState({ recurringRidesModalVisible: false })
        }
          ...
          <View style={styles.returnModalContent}>
            ...
            <View style={styles.closeWrapper}>
              <TouchableOpacity
                ...
                onPress={() => this.setRecurringRidesModalVisibility(false)}
              >
      ...
      <View style={styles.detail}>
        ...
        <AppText color={Colors.text.darkGray}>
          ...
          {trip.type === FEED_TYPE_OFFER && (
            <AppText>
              {' '}
              {trans('feed.offers')} {trip.seats}{' '}
              {trip.seats > 1 ? trans('feed.seats') : trans('feed.seat')}{' '}
            </AppText>
          )}
          {trip.type === FEED_TYPE_WANTED && (
            <AppText> {trans('feed.asks_for_a_ride')}</AppText>
          )}
        </AppText>
        <Heading fontVariation="bold" style={styles.fromTo}>
          {trip.TripStart.name ? trip.TripStart.name : UcFirst(trip.direction)}{' '}
          - {trip.TripEnd.name ? trip.TripEnd.name : UcFirst(trip.direction)}
        </Heading>
  ...
  renderParticipantsAction() {
    ...
    return (
      <View>
        ...
        />
        {trip.muted ? (
          <ModalAction label={trans('detail.unmute')} onPress={this.onUnmute} />
        ) : (
          [
            <ModalAction
              key="mute_two_hours"
        ...
        <ModalAction
          label={trans('detail.embed_with_html')}
    ...
    const participate = [
      ...
      >
        ...
        <Text style={[actionSheetMenu.actionLabel, { color }]}>
          {trans('detail.share_your_live_location')}
        </Text>
      </TouchableOpacity>,
      ...muteActions,
      <TouchableOpacity
    ...
    if (trip.experienceStatus === EXPERIENCE_STATUS_PENDING) {
      return (
        <View style={styles.experienceWrapper}>
          ...
          <View style={{ flex: 1 }}>
            <AppText>
              {trans('detail.someone_has_already_created_an_experience')}
            </AppText>
          </View>
        </View>
      );
  ...
  renderDeletedModal = () => {
    ...
    return (
      <InfoModal
        ...
        onConfirm={() =>
          this.setState(
            { deletedModal: false, confirmModalVisibility: false },
            () => this.navigateOnDelete(),
          )
        }
        confrimTextColor={Colors.text.blue}
  ...
  renderTripNotAvailable = () => {
    ...
    return (
      <InfoModal
        ...
        message={message}
        onConfirm={() =>
          this.setState({ notAvailableModal: false }, () => {
            this.isNotAvailableModalDisplayed = true;
            navigation.goBack();
          })
        }
        confrimTextColor={Colors.text.blue}
